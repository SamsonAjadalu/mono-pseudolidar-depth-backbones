...
OUT_DIR = KITTI_ROOT / "training" / "velodyne_exp2"
...
def parse_calib_file(calib_path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Parses a KITTI calib file and returns:
      ...
      - Tr (4x4) velo->cam0 transform (homogeneous)
    """
...
def cam_to_velo(points_cam: np.ndarray, R0: np.ndarray, Tr: np.ndarray) -> np.ndarray:
    """Transforms points from rectified camera coordinates to Velodyne coordinates."""
...
def clean_and_thin_cloud(points_4d: np.ndarray) -> np.ndarray:
    """Removes outliers and optionally downsamples to a fixed point count."""
...
def pad_to_multiple(img: torch.Tensor, multiple: int = 32) -> tuple[torch.Tensor, int, int]:
    ...
    Returns (padded, original_h, original_w).
    """
    _, _, h, w = img.shape
...
def load_frame_ids() -> list[str]:
    """Loads frame ids from the split file if present, otherwise scans the image directory."""
...
def main() -> None:
    ...
    print(f"Loading NeWCRFs (encoder={ENCODER}) | scale={SCALE_FACTOR} | max_depth={MAX_DEPTH_VALID}m")
    ...
    print(f"Generating {len(frame_ids)} point clouds -> {OUT_DIR}")
    ...
    for frame_id in tqdm(frame_ids):
        ...
        with torch.no_grad():
            depth = model(img_t)[:, :, :orig_h, :orig_w].cpu().numpy().squeeze()
        ...
        intensity = (img_gray[mask].astype(np.float32) / 255.0).reshape(-1, 1)
