...
OUT_DIR = KITTI_ROOT / "training" / "velodyne_smart_gray"
...
def decode_car_mask(frame_id: str, mask_data: dict, h: int, w: int) -> np.ndarray:
    """
    Builds a boolean mask for "car-like" pixels from saved Mask R-CNN outputs.
    This is only used to bias sampling; intensity still comes from grayscale.
    """
    is_car = np.zeros((h, w), dtype=bool)
    ...
    for i in range(n):
        if scores[i] <= CAR_SCORE_THRESHOLD:
            continue
...
def main() -> None:
    ...
    print(f"Generating smart-grayscale pseudo-LiDAR ({TARGET_POINTS} pts max) -> {OUT_DIR}")
    ...
    for frame_id in tqdm(frame_ids):
        ...
        # Car mask used only to bias sampling
        is_car_pixel = decode_car_mask(frame_id, mask_data, h, w)
        ...
        intensity = (img_gray[valid].astype(np.float32) / 255.0).reshape(-1, 1)
        points_4d = np.hstack([points_velo, intensity]).astype(np.float32, copy=False)

        car_status = is_car_pixel[valid]
        num_points = points_4d.shape[0]

        # Sampling: keep all car points if possible, then fill with background
        if num_points > TARGET_POINTS:
            fg_idx = np.where(car_status)[0]
            bg_idx = np.where(~car_status)[0]

            if fg_idx.size >= TARGET_POINTS:
                choice = np.random.choice(fg_idx, TARGET_POINTS, replace=False)
            else:
                need = TARGET_POINTS - fg_idx.size
                if bg_idx.size > need:
                    bg_choice = np.random.choice(bg_idx, need, replace=False)
                    choice = np.concatenate([fg_idx, bg_choice])
                else:
                    choice = np.arange(num_points)

            np.random.shuffle(choice)
            points_4d = points_4d[choice]
